<x-app-layout>
    <div class="flex w-full items-center justify-center h-screen">
        ...
        <form action="{{ route('categories.update') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-80">
            @csrf
            @method('PUT')
            <label>Name</label>
            <input type="text" name="name" value="{{ $category->name }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
    </div>
