                    ...
                    <tr class="bg-white hover:bg-gray-100 transition-all duration-150">
                        ...
                        <td class="px-6 py-3">
                            @foreach ($work->artists as $artist)
                                <a href="{{ route('artists.edit', $artist) }}" class="group text-xs text-center text-gray-900 hover:text-yellow-600 flex items-center gap-1">
                                    <svg class="text-gray-900 group-hover:text-yellow-600 h-4 w-4" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6m-7 1l9-9m-5 0h5v5"/></svg>
                                    {{ $artist->name }}
                                </a>
                            @endforeach
                        </td>
                        <td class="px-6 py-3">
                            <span class="text-xs text-center text-gray-900 bg-gray-100 border py-1 px-2 rounded-full">{{ $work->category->name }}</span>
                        </td>
